...
useEffect(() => {
  ...
  // Initialize waveform if not already
  if (!waveform.current) {
    waveform.current = WaveSurfer.create({
      container: waveFormRef.current,
      waveColor: "#ccc",
  ...
  useEffect(()=>{
      if(audioMessage){
        ...
        return()=>{
          audioMessage.removeEventListener("time-update",updatePlaybackTime)
        }
      }
    },[audioMessage])
  ...
  return <div  className={`flex items-center gap-5 text-white px-4 pr-2 py-4 text-sm rounded-md ${
        ...
        <div>
          <Avatar type ="lg" profileImage ={currentChatUser?.profilePicture}/>
        </div>
